...
def test_defaults_when_no_config(tmp_path: Path):
    ...
    assert cfg.enabled_tools == ["read_file", "write_file", "execute_shell", "run_tests", "search_code", "list_files", "restore_file", "git", "web_fetch"]
...
def test_lists_are_appended(tmp_path: Path):
    ...
    cfg = mgr.load()
    # Merge order: defaults (9 tools), + global (appends 2), + project (appends 2) = 13.
    assert len(cfg.enabled_tools) == 13
    ...
    assert cfg.enabled_tools.count("web_fetch") == 1  # only in defaults
