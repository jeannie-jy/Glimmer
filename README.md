...
<p align="center">
  <img src="docs/读取网站.png" alt="Agent 读取网站示例" width="800" />
</p>

Agent 通过 `web_fetch` 工具**抓取网页并转为可读文本**——HTML 自动提取标题、描述与正文（滤除脚本与隐藏内容），工具调用卡片展示状态码与内容，内网/云元数据地址由 SSRF 护栏硬封锁。
...
| `web_fetch` | 抓取公网网页文本（HTML 自动转纯文本：标题 + 描述 + 正文，去脚本/隐藏内容；content 上限 512KB、stdout 截断 4096 字符、重定向最多 5 跳；仅 http(s) 80/443；私网/云元数据地址硬封锁） | `url` |
